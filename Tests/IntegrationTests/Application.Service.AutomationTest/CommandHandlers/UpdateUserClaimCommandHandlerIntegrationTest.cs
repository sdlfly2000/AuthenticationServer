...
namespace Application.Service.AutomationTest.CommandHandlers
{
    ...
    public class UpdateUserClaimCommandHandlerIntegrationTest
    {
        ...
        public static void ClassCleanup()
        {
            ...
            {
                _dbContext.Database.ExecuteSqlRaw($"DELETE FROM [IdentityTest].[dbo].[Claim] WHERE UserId = '{UserId}' AND Name = '{ClaimTypes.Email}'");
                _dbContext.Dispose(); 
            }
        ...
        public async Task Given_UserIdAndClaimTypeAndNewClaimValue_When_Handle_Then_ResponseReturn()
        {
            // Arrange
            ...
            Assert.IsTrue(user.Claims.Any(c => c.Name.Equals(claimType) && c.Value.Equals(NewClaimValue)));
        }
    }
}